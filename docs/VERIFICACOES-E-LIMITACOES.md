...
- **Carregamento do arquivo de alvos**: `proof.html` busca `targets/proof/card.mind` (o alvo oficial de exemplo do mind-ar-js) com sucesso; `index.html` busca `targets/treasure-hunt.mind` (nossos 5 alvos) com sucesso.
- **Validação estrutural do `treasure-hunt.mind` pelo parser real do MindAR**: chamamos `Compiler.importData()` (a mesma classe usada em tempo de execução pelo jogo) diretamente no navegador sobre o arquivo compilado e confirmamos os alvos, cada um 1000×1000, cada um com dados de *matching* (11 níveis de pirâmide) e de *tracking* presentes. Isso comprova que o `.mind` gerado é válido e carregável pelo MindAR de verdade — não é um arquivo vazio, fictício ou só renomeado. (Verificação original feita com 3 alvos; após a expansão para 5 pistas, `npm run compile-targets` recompilou as 5 imagens com sucesso — log do script confirma as 5 imagens 1000×1000 carregadas na ordem certa — mas a validação estrutural via `Compiler.importData()` no navegador não foi reexecutada nesta sessão.)
...
- **Modo demonstração — fluxo completo** (verificado originalmente com 3 pistas; a mecânica é a mesma, só escalada para 5): nome da equipe → pista 1 a N → "Simular descoberta" em cada uma → inventário mostrando coletados/pendentes corretamente → tela final com baú 3D → animação de abertura da tampa → mensagem final, nome da equipe e pontuação → reiniciar com confirmação → `localStorage` de demo e de partida real corretamente limpos e **nunca compartilhados entre si**. Após a expansão para 5 pistas, o fluxo de 5 coletas + revelar dica ainda precisa ser reexecutado manualmente neste ambiente (ver `TESTE-MANUAL.md`, seções 3 e 4).
- **Regras de pontuação/sequência/bloqueio (namespace real)**: verificado chamando diretamente `coletarTesouro()` (a mesma função usada pela UI) — coletar o alvo certo avança a pista e soma 100 pontos; coletar um alvo errado/fora de ordem é bloqueado (`ok:false`, nada muda). Com 5 pistas, o máximo passa a ser 500 pontos (100 × 5), menos 30 por cada dica extra revelada (`revelarDica()`, nova função, ainda não reexecutada manualmente nesta sessão — mesma limitação do parágrafo acima).
